...
class OrderItemController extends RController
{
	/**
	 ...
	 * Manages all models.
	 */
	public function actionAdmin($orderid)
	{
		$model=new OrderItem('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($orderid))
		{	$orderid = intval($orderid);
			$model->attributes=array('order_id'=>$orderid);
			$order = Yii::app()->db->createCommand()
				    ...
				    ->queryRow();
			if (empty($order)) {
				throw new CHttpException(404,'没有找到订单信息！');
			}
		}
